...
class Enigma {
  ...
  stepRotors() {
    // In the real Enigma, rotors step in sequence with proper timing
    // Check if middle rotor is at notch (for double-stepping)
    const middleAtNotch = this.rotors[1].atNotch();
    
    // Check if right rotor is at notch (will trigger middle rotor step)
    const rightAtNotch = this.rotors[2].atNotch();
    
    // Step rotors in proper sequence:
